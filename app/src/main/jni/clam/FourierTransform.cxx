...
{
	_spectrum = new double[2*mFrameSize];
	#if USE_FFTW3
...
void FourierTransform::doItGeneric(const T * input)
{
	...
	{
	  //We need to copy this? Seriously, guys?
	  for (unsigned long i=0; i<mFrameSize; i++)
	    {
	  ...
	  unsigned j = mFrameSize*2-1;
	  for (int i=1; i<mFrameSize; i+=2, j-=2)
	    {
	      _spectrum[j-1] = _complexOutput[i/2][0];
	      _spectrum[j] = _complexOutput[i/2][1];
	    }
	}
#else
	if (mIsComplex)
