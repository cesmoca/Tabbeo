...
public class Base64Coder {
    ...
    public static String encodeLines(byte[] in, int iOff, int iLen, int lineLen) {
        ...
        int bufLen = ((iLen + 2) / 3) * 4 + lines * Base64Coder.systemLineSeparator.length();
        StringBuilder buf = new StringBuilder(bufLen);
        int ip = 0;
        while (ip < iLen) {
            int l = Math.min(iLen - ip, blockLen);
            buf.append(encode(in, ip, l));
            buf.append(Base64Coder.systemLineSeparator);
            ip += l;
        }
        return buf.toString();
    }
     ...
     * @throws IllegalArgumentException If the input is not valid Base64 encoded data.
     */
    public static byte[] decode(char[] in, int iLen) {
        if (iLen % 4 != 0)
